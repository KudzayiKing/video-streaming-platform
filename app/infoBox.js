    ...
    return (
        <div
            ...
            <div className="z-50  fixed top-0 w-screen h-screen lg:w-[60%] lg:h-[100%] lg:bottom-10 xl:w-[60%] rounded-t-2xl  bg-gray-900 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-[0.50]  shadow-lg  xl:bottom-0  xl:rounded-t-3xl 2xl:w-[60%] 2xl:bottom-0">
                ...
                />
                {/* Buttons section */}
                <div className="z-30 flex justify-between space-x-2 items-center pt-4 h-15 xl:pt-2">
                    <div onClick={() => { checkFavs(id) }} className="flex flex-col justify-center items-center w-32 ">
                        {favourites.filter((item) => item.id == id).length ? <FaHeart size={22} color={'orange'} className='min-[1920px]:w-[35px] min-[1920px]:h-[35px] xl:w-[18px] xl:h-[18px]' /> : <FaRegHeart size={22} color={'#FFFFFF'} className='min-[1920px]:w-[35px] min-[1920px]:h-[35px] xl:w-[18px] xl:h-[18px]' />}

                        <p className="text-xs pt-1 text-shadow-lg font-medium text-white min-[1920px]:text-lg xl:text-xs ">My List</p>
                    </div>
                    <div>
                        <button className="flex justify-center shadow-xl items-center p-3 h-11 w-40 min-[1920px]:w-[220px] min-[1920px]:h-[50px] xl:h-[34px] text-black text-lg rounded-3xl  font-bold bg-white space-x-1 ">
                            <FaPlay size={15} />
                            <p>Play</p>
                        </button>
                    </div>
